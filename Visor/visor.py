...
# Lectura del archivo de benchmark al día
df_vu_bench = pd.read_excel(ruta_opt + '/data/benchmark/' + archivo_vu, skiprows=2, index_col=0)

df_ret_diarios_aj = transform_benchmark_al_dia(df_vu_bench, window ='D')

# df_ret_diarios_aj = pd.read_excel(ruta_opt + '/data/benchmark/ret_prom_sem_aj.xlsx', index_col=0)

#archivo de optimización
df_portafolios = leer_archivo_optimizacion(ruta_opt +'/OPTIMIZACIONES/INDICES_OPT.xlsx', 'INDICES')

tipos_portafolios = df_portafolios['Type_Portfolio'].unique()

tipos_optimizacion = np.array(['Information Ratio', 'Máximiza Utilidad', 'Sharpe Ratio'])

periodos_vol = np.array(['1M', '3M', '6M', '9M', '1Y'])

app.layout = html.Div([
    dcc.Store(id='df-portafolios-seleccionados-store'),
    dcc.Store(id='df-portafolios-seleccion-store'),
    dcc.Store(id='portafolio-seleccionado-store'),

    #---------- Header Visor ----------#
    html.Div([
        html.Img(src='https://bancolombia.sharepoint.com/:i:/r/sites/co-fnx/logos/Colombia/Grupo%20Bancolombia%20Capital/GBC_logotipo_horizontal_positivo.png?csf=1&web=1&e=31t3Zb', style={'height':'70px', 'float':'left'}),
        html.H4('Dirección de Estrategia - Asset Management', style={'color': colores_corporativos['black'], 'clear':'left'}),
        html.P(id='date-time', style={'color': colores_corporativos['black']}),
        dcc.Interval(
            id='interval-component',
            interval=60*1000, # en milisegundos
            n_intervals=0
        ),
    ], style={'padding': '5px 10px', 'backgroundColor': colores_corporativos['white'], 'color': colores_corporativos['black']}),
    #------------------------------------#
    ...
    html.Div([
        ...
        html.Div([  
            ...
            html.Div([ 
                 ### header Selección ###
                html.Div([ 
                    html.H3('Selección', style={'color': colores_corporativos['white']}),
                ], style={'padding': '5px 10px', 'backgroundColor': colores_corporativos['black'], 'color': colores_corporativos['white']}),
                #######################
                html.Div([
                    ### dropdown Selección ###
                    ...
                    dcc.Dropdown(
                        ...
                        options=[{'label': i, 'value': i} for i in tipos_portafolios],
                        style={'width': '300px','margin-bottom': '10px'},  
                    ),

                    html.Label('Portafolio:'),
                    dcc.Dropdown(
                        id='portafolio-dropdown',
                        style={'width': '300px','margin-bottom': '10px'}, 
                    ),

                    html.Label('Tipo de Optimización:'),
                    dcc.Dropdown(
                        id='optimizacion-dropdown',
                        options=[{'label': i, 'value': i} for i in tipos_optimizacion],
                        style={'width': '300px','margin-bottom': '10px'}, 
                    ),

                    html.Label('Períodos Volatilidad:'),
                    dcc.Dropdown(
                        id='volatilidad-dropdown',
                        options=[{'label': i, 'value': i} for i in periodos_vol],
                        style={'width': '300px','margin-bottom': '10px'}, 
                    ),
                    #######################
                ...
                html.Div([
                ### Botón Optimización ###
                html.Button('Optimizar', id='optimize-button', style={
                    'background-color': colores_corporativos['yellow'],
                    'color': colores_corporativos['black'], 
                    'margin': '10px',
                ...
                #######################
                ### Botón Resetear ###
                html.Button('Resetear', id='reset-button', style={
                    'display': 'none', 
                    'background-color': colores_corporativos['yellow'],
                    'color': colores_corporativos['black'], 
                    'margin': '10px',
                ...
                #######################
                ], style={'padding': '10px', 'backgroundColor': colores_corporativos['white'], 'color': colores_corporativos['black'], 'border': '1px solid ' + colores_corporativos['black']}),      
           ...
           #------------- Portafolio Seleccionado -------------#
            html.Div([
                html.Div([
                    html.Div([     
                        html.Div([ 
                                html.H3('Portafolio Seleccionado',id='port-selection-header', style={'color': colores_corporativos['white'], 'display': 'block'}),
                            ], style={'padding': '3px 10px', 'backgroundColor': colores_corporativos['black'], 'color': colores_corporativos['white']}),

                        html.Div(id='output-select', style={'padding': '0px 0px 20px 1px'}),

                    ], id='parent-div-select',style={'display': 'none', 'padding': '5px','justify-content': 'flex-start', 'align-items': 'flex-start', 'backgroundColor': colores_corporativos['white']}),

                    #------------- Restricciones Portafolio Seleccionado -------------#
                    html.Div([     
                        html.Div([ 
                                html.H3('Restricciones del Portafolio',id='port-const-header', style={'color': colores_corporativos['white'], 'display': 'block'}),
                            ], style={'padding': '3px 10px', 'backgroundColor': colores_corporativos['black'], 'color': colores_corporativos['white']}),

                        html.Div(id='output-constraints', style={'padding': '0px 0px 20px 1px'}),

                    ], id='parent-div-cons',style={'display': 'none', 'padding': '5px','justify-content': 'flex-start', 'align-items': 'flex-start', 'backgroundColor': colores_corporativos['white']}),
                ], style={'display': 'flex'}),
                #---------------------------------------------------#
    ...
    ], style={'display': 'flex', 'padding':'10px','justify-content': 'flex-start', 'align-items': 'flex-start', 'backgroundColor': colores_corporativos['white']}),

    #---------- Contenido Correlaciones ----------#
    html.Div([
        html.Div([html.H3('Correlación',style={'color': colores_corporativos['white'],'display': 'block'}),
                ], style={'padding': '3px 10px', 'backgroundColor': colores_corporativos['black'], 'color': colores_corporativos['white']}),
    ...
    html.Div([
        html.Div([html.H3('Optimización',style={'color': colores_corporativos['white'],'display': 'block'}),
                ], style={'padding': '3px 10px', 'backgroundColor': colores_corporativos['black'], 'color': colores_corporativos['white']}),
        html.Div([
    ...
    html.Div([
        ...
        html.Div([html.H3('Gráficas',style={'color': colores_corporativos['white'],'display': 'block'}),
                ], style={'padding': '3px 10px', 'backgroundColor': colores_corporativos['black'], 'color': colores_corporativos['white']}),     
        ## Allocation ##
        html.Div([
    ...
    #------------------------------------#
], style={'display': 'inline','backgroundColor': colores_corporativos['white']})


'''-------------------------------Callbacks ------------------------------- '''
# Actualizar la fecha y hora
@app.callback(Output('date-time', 'children'),
              [Input('interval-component', 'n_intervals')])
def update_date_time(n):
    return 'Fecha y hora actual: ' + datetime.datetime.now().strftime("%Y-%m-%d %H:%M")

# Actualizar el dropdown de portafolios
@app.callback(
    Output('portafolio-dropdown', 'options'),
    Output('df-portafolios-seleccionados-store', 'data'),
    Input('estrategia-dropdown', 'value'),
    prevent_initial_call=True,
)
def update_portafolio_dropdown(value):
    df_aux = df_portafolios.copy()
    portafolios = df_aux.query("Type_Portfolio == @value")['Ticket_Portfolio'].unique()
    df_portafolios_seleccionados = df_aux.query("Type_Portfolio == @value")
    return [{'label': i, 'value': i} for i in portafolios], df_portafolios_seleccionados.to_dict()
...
def update_const(value, n_clicks):  
    ...
    return dash_table.DataTable(
        ...
        style_header={
            'backgroundColor': colores_corporativos['black'],
            'color': colores_corporativos['white'],
        },
        style_data={
            'border': '1px solid ' + colores_corporativos['black']
        },
        editable=True,
...
def show_headers(data):
    return {'display': 'block', 'padding': '5px','justify-content': 'flex-start', 'align-items': 'flex-start', 'backgroundColor': colores_corporativos['white']}
...
def show_headers(data):
    return {'display': 'block', 'background-color': colores_corporativos['yellow'],
            'color': colores_corporativos['black'], 'margin': '10px',
            'border-radius': '5px',  # Agrega redondez al botón
            'font-weight': 'bold',  # Hace que el texto sea negrita
            'border': 'none',  # Quita los bordes
            'box-shadow': '0px 8px 15px rgba(0, 0, 0, 0.1)',  # Agrega sombra
...
# frontera Eficiente 
@app.callback(
    Output('output-container-graph_ef', 'children'),
    Input('my-table-opt', 'data'),
    prevent_initial_call=True,
)
def update_graphs_efficient_front(opt_data):
